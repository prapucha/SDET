...
Feature: ELearning Test Case
  ...
  Scenario: To verify whether application allows teacher to add project details
    ...
    And enter valid credential in title text box "e learning"
    And enter valid credential in sub title text box "e learning project"
    And click on save blog button
    And click on project link
    And click on new task icon
    And enter valid title in title text box "design"
    And click on save button
    And click on Role management icon
    And click on add a new role icon
    And enter valid role in title text box "developer"
    And click on save button for Role
    And click on Assign role icon
    And select student / teacher name in user list box "test teacher"
